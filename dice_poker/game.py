...
def match(roll: np.ndarray):
    ...
    # Add straight
    if len(unique) == 5 and not all(np.isin(roll, v).any() for v in [1, 6]):
...
class Game:
    ...
    def __str__(self):
        return str(self._state).replace("'", "").replace("[", " ").replace("]", "")
    ...
    def turn(self):
        ...
        for _ in range(3):
            ...
            move = yield GameState(current_roll, options, self.state)
            if move.type == PLACE:
                x, y = move.value
